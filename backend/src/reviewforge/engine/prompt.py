...
这是 correctness 之外的第二遍审查，只寻找能从 diff 直接证明的语言、框架和测试运行时陷阱：
- 重复定义覆盖实现、抽象/接口方法缺失、框架发现规则失效
- 未等待的线程/Promise、固定 sleep 竞态、mock/monkeypatch 污染后续行为
- getter/computed/read API 产生写副作用，资源或 listener 生命周期错误
- React key、async callback、nil/null/unwrap/panic 等会造成具体错误结果的语言语义

每条 finding 必须写明触发路径和实际失败结果。不要报告命名（除非破坏框架发现或公共 API）、
类型注解、文档、复杂度、函数长度、接口大小、clone/mut 偏好、重构、格式、import 排序、
“更惯用”的替代写法、微优化或仅仅缺少测试。无法证明可观察失败时输出空 findings。"""
