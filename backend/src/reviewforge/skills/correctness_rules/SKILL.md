...
3. Trace one concrete input or execution path to an observable wrong result. Report only when that path is supported by changed code or retrieved repository evidence.
4. Check boundaries: empty/null values, first/last item, error returns, cancellation, ordering, concurrent access, partial success, and rollback.
5. Prefer a small exact fix that restores the existing contract.
